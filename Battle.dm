mob
	proc
		...
		Battle()
			var/monstername="[monstergroup[1]]"
			monstername=copytext(monstername,1,length(monstername)-1)
			if(usr.defend == 1)
				usr.defense /= 2
				usr.defend = 0
			if(usr.poisoned == 1)
			...
			if(usr.mem == 0)
				...
				usr.mem = 1
			switch(input("You encountered [monstergroup.len] [monstername][plural(monstergroup)]!  Command?","Battle!") in list("Attack","Spell","Item","Defend","Run"))
				if("Attack")
					Attack()
				if("Spell")
					Spell()
		...
		Attack()
			...
			if(damage <= 0)
				...
				if(onedmgchance == 3)
					...
					usr << 'attack.wav'
					sleep(10)
					usr << "You barely hit the [enemy], scraching it for 1 damage!"
					usr << 'hit.wav'
					enemy.hp -= 1
					sleep(10)
					if(src.hp <= 0)
			...
			if(critchance == 9)
				...
				if(critdamage <= 0)
					...
					sleep(10)
				if(critdamage >> 0)
					usr << "You hit the [enemy] for [critdamage]!"
					usr << 'hit.wav'
					enemy.hp -= critdamage
					sleep(10)
					if(src.hp <= 0)
			...
			if(chancehit > 0 && damage > 0)
				...
				usr << 'attack.wav'
				sleep(10)
				usr << "You hit the [enemy] for [damage] damage!"
				usr << 'hit.wav'
				enemy.hp -= damage
				sleep(10)
				if(enemy.hp <= 0)
		...
		Spell()
			...
			if(isnull(usr.spelllist))
				usr << "<font color = blue><b>You don't have any spells or skills!"
				Battle()
			else if(usr.diseased == 1)
				usr << "<font color = blue><b>You are too weak to cast spells or use skills!"
				Battle()
		...
		Deathcheck(mob/enemy)
			if(enemy.hp <= 0)
				usr << "You have killed the [enemy]!"
				usr << 'winbattle.wav'
				sleep(10)
				usr << "You recieve [enemy.expreward] exp and [enemy.gold] gold!"
				if(enemy.jobexpreward >= 1)
					usr << "<font color = blue><b>You recieve [enemy.jobexpreward] job exp!"
			...
			if(usr.hp <= 0)
				var
					goldlost = usr.gold/2
				usr << "<b><font color = blue>You died!  The [enemy] runs away with half your gold!"
				usr.gold -= goldlost
